...
let importfunction = require('./tests/config/import');

// The below module is used for cucumber html report generation
const reporter = require('cucumber-html-reporter');
const currentTime = new Date().toJSON().replace(/:/g, '-');
const sourceSpecDirectory = 'tests/features/e2e/Main';
const jsonTmpDirectory = 'tests/reports/json/tmp/';

const cookieConcerns = require('./tests/utility/protocolCommands/cookieConcernCommand.json').jsonWireActions;

let argv = require('yargs')
    .usage('Usage: npm run test --[options]')
    .options({
        browser: {
            alias: 'browser',
        ...
        loggingLevel: {
            alias: 'loggingLevel',
            describe: 'Webdriverio Logging Type',
            default: 'silent'
        }
    })
    .argv;
if(argv.ff !== '**') {
    featureFilePath = `${sourceSpecDirectory}/${argv.ff}.feature`;
}
if(argv.ff == '**') {
    featureFilePath = `${sourceSpecDirectory}/**`;
}
exports.config = {
    environment: argv.envi,
    //
    // ====================
    ...
    beforeCommand: function (commandName) {
        if(cookieConcerns.includes(commandName)) {
            let cookieLocator = '//*/button[text()="Agree" or @title="Agree"]';
            if($(cookieLocator).isDisplayed()) {
                return $(cookieLocator).doubleClick();
            }
        }
    },
    /**
